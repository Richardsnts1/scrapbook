...
function renderizaMensagem(){
    ...
    for(var i = 0;i<mensagens.length;i++){
        ...
        scrapfieldElementBox.setAttribute('class',`-webkit message-cards card text-white bg-dark m-2 col-3 caixa${position}`);
        scrapfieldElementHeader.setAttribute('class','card-header');
        scrapfieldElementBody.setAttribute('class','card-body');
        message.setAttribute('class','card-text');6
...
function criaMensagem() {
    ...
    //setta os atributos da caixa de acordo com o bootstrap
    scrapfieldElementBox.setAttribute('class',`-webkit message-cards card text-white bg-dark m-2 col-3`);
    scrapfieldElementHeader.setAttribute('class','card-header');
    scrapfieldElementBody.setAttribute('class','card-body');
    message.setAttribute('class','card-text');

    //verifica se os campos não estão em branco para prosseguir com o código
    if((document.getElementById("titleInput").value && document.getElementById("messageField").value) !== ""){ 
        if(addButton.classList.contains('btn-danger')){ //verifica se o botão está vermelho
        ...
        addButton.innerHTML = 'Por favor, escreva algo no título e/ou no campo de mensagem!'; //muda a mensagem para <
